    ...
    class Task_2 : Task
        {
            ...
            protected override void ParseText(string text)
            {
                ...
                string parsedText = string.Join(" ", words); 
            ...
            {
                string[] words = encryptedMessage.Split(new char[] { ' ', '.', ',', ':', ';', '!', '?' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        protected override void ParseText(string text)
        {
            ...
            foreach (var Word in Words)
            {
                if (newLine.Length + Word.Length + 1 > maxLineWidth)
                {
                    ...
                    if (numGaps >= 0)
                    {
                        int totalSpacesToAdd = maxLineWidth - newLine.Length;
                        int spacesBetweenWords = totalSpacesToAdd / numGaps;
                        int extraSpaces = totalSpacesToAdd - spacesBetweenWords;
